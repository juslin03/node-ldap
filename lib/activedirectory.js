...
function getUserQueryFilter(username) {
  log.trace('getUserQuery Filter(%s)', username);
  var self = this;
  if (!username) return('|(objectCategory=User)(objectClass=user)(objectClass=person)');
  if (isDistinguishedName.call(self, username)) {
    return('(&(|(objectCategory=User)(objectClass=user)(objectClass=person))(distinguishedName='+parseDistinguishedName(username)+'))');
  }
  return('(&(|(objectCategory=User)(objectClass=user)(objectClass=person))(|(sAMAccountName='+username+')(userPrincipalName='+username+')(uid='+username+')(mail='+username+')))');
}
...
function getGroupQueryFilter(groupName) {
  ...
  if (! groupName) return('|(objectCategory=Group)(objectClass=*)');
  if (isDistinguishedName.call(self, groupName)) {
    return('(&(|(objectCategory=Group)(objectClass=*))(distinguishedName='+parseDistinguishedName(groupName)+'))');
  }
  return('(&(|(objectCategory=Group)(objectClass=*))(cn='+groupName+'))');
}
...
function getGroupMembershipForDN(opts, dn, stack, callback) {
  ...
  const uidExist = parseDistinguishedName(dn).includes('uid');
  const uidOK = uidExist ? parseDistinguishedName(dn) : `member=${parseDistinguishedName(dn)}`;
  opts = _.defaults(_.omit(opts || {}, 'filter', 'scope', 'attributes'), {
    filter: `(${uidOK})`,
    scope: 'sub',
    attributes: joinAttributes((opts || {}).attributes || defaultAttributes.group, [ 'groupType', 'uid' ])
  });
...
ActiveDirectory.prototype.findUsers = function findUsers(opts, includeMembership, callback) {
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    if (err) {
      console.log('error search %s', err.message)
      if (callback) callback(err);
      return;
    }
